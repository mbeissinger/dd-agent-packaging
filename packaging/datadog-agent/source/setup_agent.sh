...
dogweb_reporting_failure_url="https://netsil.com/agent_stats/report_failure"
email_reporting_failure="support@netsil.com"
...
A notification has been sent to Netsil with the content of $logfile.
...
unknown_error() {
  ...
  report_manual
#  while true; do
#    read -p "Do you want to send a failure report to Netsil (Content of the report is in $logfile)? (y/n)" yn
#    case $yn in
#        [Yy]* ) report; break;;
...
#eval $dl_cmd $dd_base/virtualenv.py https://raw.githubusercontent.com/pypa/virtualenv/14.0.6/virtualenv.py >> $logfile 2>&1
eval $dl_cmd $dd_base/virtualenv-$VIRTUALENV_VERSION.tar.gz https://pypi.python.org/packages/source/v/virtualenv/virtualenv-$VIRTUALENV_VERSION.tar.gz >> $logfile 2>&1
tar xvfz $dd_base/virtualenv-$VIRTUALENV_VERSION.tar.gz -C $dd_base >> $logfile 2>&1
rm $dd_base/virtualenv-$VIRTUALENV_VERSION.tar.gz

if [ "$is_openshift" = "1" ]; then
    $PYTHON $dd_base/virtualenv-$VIRTUALENV_VERSION/virtualenv.py --no-pip --no-setuptools --system-site-packages $dd_base/venv >> $logfile 2>&1
#    $PYTHON $dd_base/virtualenv-$VIRTUALENV_VERSION/virtualenv.py --system-site-packages $dd_base/venv >> $logfile 2>&1
else
    $PYTHON $dd_base/virtualenv-$VIRTUALENV_VERSION/virtualenv.py --no-pip --no-setuptools $dd_base/venv >> $logfile 2>&1
#    $PYTHON $dd_base/virtualenv-$VIRTUALENV_VERSION/virtualenv.py --no-site-packages $dd_base/venv >> $logfile 2>&1
fi
...
eval $dl_cmd $dd_base/ez_setup.py https://bitbucket.org/pypa/setuptools/raw/bootstrap/ez_setup.py >> $logfile 2>&1
$dd_base/venv/bin/python $dd_base/ez_setup.py >> $logfile 2>&1
eval $dl_cmd $dd_base/get-pip.py https://bootstrap.pypa.io/get-pip.py >> $logfile 2>&1
...
eval $dl_cmd $dd_base/requirements.txt https://raw.githubusercontent.com/mbeissinger/dd-agent/$tag/requirements.txt  >> $logfile 2>&1
...
eval $dl_cmd $dd_base/requirements.txt https://raw.githubusercontent.com/mbeissinger/dd-agent/$tag/requirements-opt.txt  >> $logfile 2>&1
...
eval $dl_cmd $dd_base/agent.tar.gz https://github.com/mbeissinger/dd-agent/tarball/$tag >> $logfile 2>&1
print_done
...
if [ $apikey ]; then
    if [ -e $dd_base/agent/datadog.conf ]; then
        printf "Found existing datadog.conf file, modifying api_key....." | tee -a $logfile
        sed "s/api_key:.*/api_key: $apikey/" $dd_base/agent/datadog.conf > $dd_base/agent/datadog.conf 2>> $logfile
    ...
    if [ -e $dd_base/agent/datadog.conf ]; then
        printf "No api key set. Using the existing configuration file....." | tee -a $logfile
    else
        printf "No api key set. Using example configuration file, you need to modify with api_key later....." | tee -a $logfile
        cp $dd_base/agent/datadog.conf.example > $dd_base/agent/datadog.conf 2>> $logfile
        chmod 640 $dd_base/agent/datadog.conf
    fi
fi
...
if [ "$unamestr" = "Darwin" ]; then
    ...
    sed "s|AGENT_BASE|$dd_base|; s|USER_NAME|$(whoami)|" $dd_base/agent/packaging/osx/com.datadoghq.Agent.plist.example > $dd_base/launchd/com.datadoghq.Agent.plist 2>> $logfile
fi
...
rm -rf $dd_base/virtualenv-$VIRTUALENV_VERSION >> $logfile 2>&1
